...
export const UserManual = ({ setView }) => {
  ...
  return (
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-6 pt-6 pb-12 px-4 animate-slide-up">
      {/* Page Header */}
      ...
      {/* Tabs Layout */}
      <div className="grid gap-6 md:grid-cols-[220px_1fr] w-full items-start">
        {/* Sidebar Navigation */}
        <div className="flex flex-col gap-2 shrink-0 bg-slate-900/5 p-2 rounded-2xl border border-slate-200/50">
          {tabs.map((tab) => {
            const Icon = tab.icon;
        ...
        {/* Content Pane */}
        <div className="glass-panel rounded-[32px] p-6 md:p-8 flex flex-col gap-6 shadow-xl bg-white/40 border border-white/60 min-h-[450px] w-full">
          {activeTab === 'overview' && (
          ...
          {activeTab === 'users' && (
            <div className="flex flex-col gap-5 animate-slide-up">
              ...
              <div className="surface-panel rounded-3xl p-5 flex flex-col gap-3">
                ...
                <div className="mt-4 overflow-hidden rounded-2xl border border-slate-200 shadow bg-slate-900">
                  <img src="/assets/2_patient_main.png" alt="Simulasi Layar Pasien MedSign" className="w-full h-auto max-h-[300px] object-cover" />
                  <span className="block p-2 text-[10px] text-center text-slate-400 font-semibold uppercase tracking-wider bg-slate-950">Gambar 1: Simulasi Deteksi Kamera & Translasi Pasien</span>
                </div>
              </div>
              ...
              <div className="surface-panel rounded-3xl p-5 flex flex-col gap-3">
                ...
                <div className="mt-4 overflow-hidden rounded-2xl border border-slate-200 shadow bg-slate-900">
                  <img src="/assets/3_doctor_consultation.png" alt="Simulasi Layar Dokter MedSign" className="w-full h-auto max-h-[300px] object-cover" />
                  <span className="block p-2 text-[10px] text-center text-slate-400 font-semibold uppercase tracking-wider bg-slate-950">Gambar 2: Simulasi Panel Respon Cepat & Transkrip Dokter</span>
                </div>
              </div>
            </div>
          )}
