...
export const TranslationDisplay = () => {
  ...
  const { lastDetected, cameraActive, t } = useContext(AppContext);
  ...
  if (!cameraActive) {
    ...
    return (
      ...
      <div className="glass-panel flex min-h-[140px] flex-col items-center justify-center rounded-3xl border border-dashed border-white/70 p-6 text-center">
        ...
        <p className="text-sm font-black text-slate-600">{t('cameraInactive')}</p>
  ...
  if (mode === 'model_unavailable' || mode === 'spelling_unavailable') {

    return (

      <div className="glass-panel flex min-h-[140px] flex-col items-center justify-center rounded-3xl border border-dashed border-red-300 p-6 text-center animate-pulse">

        <AlertTriangle className="text-amber-500 mb-2" size={24} />

        <p className="text-sm font-black text-slate-700">Model ML Tidak Ditemukan</p>

        <p className="mt-1 max-w-[240px] text-xs font-semibold leading-5 text-slate-500">

          Model ML belum diunggah atau belum dilatih. Silakan buka halaman Data Collection untuk melatih atau mengunggah model.
    ...
    <div className={`glass-panel flex flex-col gap-4 rounded-3xl p-6 transition-all ${
      ...
      <div className="flex flex-col items-center justify-center py-2 text-center">
        ...
        <span className={`select-all text-4xl font-black leading-tight sm:text-5xl ${
          ...
          {activeWord ? (activeWord.length === 1 ? activeWord.toUpperCase() : t(activeWord).toUpperCase()) : '—'}
        ...
        <span className="mt-2 text-[10px] font-semibold uppercase text-slate-500">
          ...
          {mode === 'spelling' ? 'Ejaan Abjad & Angka BISINDO' : mode === 'demo' ? t('heroTitle') : 'Deep Learning Inference'} - {processing_time_ms}ms
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div className="flex items-center justify-between text-[10px] font-bold">
          ...
          <span className={colorClass}>{Math.round(confidence * 100)}% - {t('confidence' + confLabel)}</span>
      ...
      {top3 && top3.length > 1 && (
        ...
        <div className="flex flex-col gap-2 border-t border-white/60 pt-3">
          ...
          <span className="text-[10px] font-bold uppercase text-slate-500">{t('alternatives')}</span>
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            {top3.slice(0, 3).map((alt, idx) => (
              ...
              >
                ...
                <span className="block truncate text-[11px] font-black uppercase text-slate-800" title={t(alt.word)}>
