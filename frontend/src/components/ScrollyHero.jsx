...
export function ScrollyHero({ setView }) {
  ...
  useEffect(() => {
    ...
    const snapToSection = (index) => {
      ...
      // Snap to the CENTER of the section's scroll runway so the last
      // section ("Komunikasi Tanpa Batas") isn't landed exactly at the
      // content boundary and skipped.
      const span = window.innerHeight * (SECTIONS.length - 1);
      const targetScroll = ((index + 0.5) / SECTIONS.length) * span;
    ...
    const handleWheel = (e) => {
      ...
      // If currently snapping, always block natural scroll to prevent browser intervention
      ...
      // If we are below the ScrollyHero (i.e. scroll is past Section 3),

      // we only intercept if the user is scrolling UP and about to enter the ScrollyHero range.

      if (currentScroll > 3 * window.innerHeight + 5) {

        if (e.deltaY < 0 && currentScroll <= 3 * window.innerHeight + 150) {

          e.preventDefault();

          if (Date.now() - lastSnapTimeRef.current > 700) {
      ...
      // At Section 3, scroll down naturally (only if not locked and cooldown elapsed)
      if (currentSectionRef.current === 3 && e.deltaY > 5) {
        if (Date.now() - lastSnapTimeRef.current < 700) {
          e.preventDefault();
          return;
        }
      ...
      if (Date.now() - lastSnapTimeRef.current < 700) return;
      ...
      if (e.deltaY > 5) {
        ...
        if (currentSectionRef.current < 3) {
          ...
          snapToSection(currentSectionRef.current + 1);

        } else {

          // At Section 3, scroll down to content below

          const targetScroll = 3 * window.innerHeight + 100;
    ...
    const handleTouchMove = (e) => {
      ...
      if (currentScroll > 3 * window.innerHeight + 5) return;

      if (currentSectionRef.current === 3) return;
    ...
    <div ref={containerRef} style={{ position: 'relative', height: '400vh' }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          background: 'linear-gradient(to bottom, transparent, #f8fafc 92%)',
          ...
          {loaded && (
            ...
            <motion.div
              ...
              style={{
                ...
                paddingTop: 24, paddingBottom: 120, // lift text clear of the bottom fade

                pointerEvents: 'none',

                opacity: scrollPct > 0.9 ? Math.max(0, (1 - scrollPct) / 0.1) : 1,
